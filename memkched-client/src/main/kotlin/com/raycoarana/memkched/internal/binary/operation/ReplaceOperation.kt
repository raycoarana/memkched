        ...
        return if (reply == Reply.DEFAULT) {
            socket.readHeader(
                headerProcess = { resultOpCode, _, _, _, _, _ ->
                    assert(resultOpCode == OpCode.REPLACE) { "Unexpected op code in response $resultOpCode" }

                    AddReplaceResult.Stored
                },
                errorProcess = { error, _ ->
                    when (error.status) {
                        KEY_NOT_FOUND -> AddReplaceResult.NotStored
                        else -> throw error.asException()
                    }
                }
            )
        } else {
            AddReplaceResult.NoReply
        }
    }
}
